...
<c:if test="${not empty param.gregcode}">
	...
	<c:if test="${param.gsort != '1'}">
		<div class="tab_sort tab_saja"><a href="/spw/local.spw?gregcode=${param.gregcode}&gsort=1&cgnum=${param.cgnum}">구매</a></div>
	</c:if>
	<c:if test="${param.gsort == '2'}">
		<div class="tab_sort tab_panda tab_sel">판매</div>
	</c:if>
	<c:if test="${param.gsort != '2'}">
		<div class="tab_sort tab_panda"><a href="/spw/local.spw?gregcode=${param.gregcode}&gsort=2&cgnum=${param.cgnum}">판매</a></div>
	</c:if>
	</div>
</c:if>
