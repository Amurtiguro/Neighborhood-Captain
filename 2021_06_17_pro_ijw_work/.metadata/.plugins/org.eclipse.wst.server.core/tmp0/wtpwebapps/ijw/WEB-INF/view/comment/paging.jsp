...
<%
	...
	if(pagecount > 0){
		...
		if(groupstart > 1){
%>
	<a href="/ijw/<%=url%>?<%=qStr%>curpage=1" class="sitebtn">◁◁</a>
	<a href="/ijw/<%=url%>?<%=qStr%>curpage=<%=(groupstart - groupsize)%>" class="sitebtn">◀</a>
<%
		}else{
%>
...
%>
	<a href="/ijw/<%=url%>?<%=qStr%>curpage=<%=(groupstart + groupsize)%>" class="sitebtn">▶</a>
	<a href="/ijw/<%=url%>?<%=qStr%>curpage=<%=pagecount%>" class="sitebtn">▷▷</a>
<%
		}else{
%>
	<span class="sitebtn">▶</span>
