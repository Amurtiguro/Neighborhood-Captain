...
<script type="text/javascript">
	...
	/* 	listForm.action="insertForm.ijw";
		listForm.submit(); */
		location.href="generalInsertForm.ijw?gsort=2";
	}
	...
	/* 상세보기 : generalDetailForm */
 	function generalDetailForm(){
		...
		$("#pList #gnum").val(gnum);
		$("#pList").attr("action", "generalDetailForm.ijw");
		$("#pList").submit();
	} 
 	/* 날짜검색 */
	function searchDate(){
	...
	<div class="list_grid">
		...
		<div class="list_grid_count">
			...
			<h5> 총 <%=count%>페이지 중 현재 <%=cur%>페이지</h5>
				...
				%>
					<li class="grid_item">
						<a href="/ijw/generalDetailForm.ijw?gnum=<%=gvo.getGnum()%>">
							<div class="grid_item_box">
								<div class="grid_img">
									<div class="grid_img_label label_panda">
										판매
									</div>
									<img src="/ijw/editImages/<%=gvo.getGrephoto() %>">
								</div>
