...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $('#dealFunc').click(function(){
	    	//alert("전송");
		    $("#detailList").attr("action", "/ijw/dealInsert.ijw");	    		
			$("#detailList").submit();	    		
	    });
	    ...
	    $('#pick').click(function(){
	    	...
	    	if(mnum!=null&&mnum.length>4){
	    		var gnum = $("#gnum").val();
	    	 	$("#detailList").attr("action", "/ijw/myPick.ijw?gnum"+gnum);
				$("#detailList").submit();
	    	}else{
	    		alert("로그인하세요.");
	    	}
	...
	function generalUpdateForm(){
		...
		$("#gnum").val(gnum);
		$("#detailList").attr("action", "generalUpdateForm.ijw");
		$("#detailList").submit();
	...
	function generalDeleteOK(){
		...
		$("#detailList").attr("action", "generalDeleteOK.ijw");
		$("#detailList").submit();
	}
	...
	function cancel(){
		...
		history.back();
		/* location.href="generalAllList.ijw"; */
...
<div class="product_detail">
	<div class="product_top">
		...
		<div class="product_info_images" style="background-image: url('/ijwv/uploadImages/<%=gvo.getGphoto()%>');" alt="상품 이미지">
			<img src="/ijw/uploadImages/<%=gvo.getGphoto() %>">
		</div>
...
<%
		...
		String url;
		if(gsort.equals("1")){
			url="/ijw/generalSList.ijw";
		}else{
			url="/ijw/generalPList.ijw";
		}
		if(mnum.equals(sessionNum)){
%>
				<a href="/ijw/generalUpdateForm.ijw?gnum=<%=gvo.getGnum() %>" class="sitebtn">수정</a>
				<a href="/ijw/generalDeleteOK.ijw?gnum=<%=gvo.getGnum() %>&gsort=<%=gvo.getGsort() %>" class="sitebtn">삭제</a>
...
%>
	...
	<%-- 댓글 --%>
	<jsp:include page="/comment/list.ijw">
