...
<div class="list_contents">
	<div class="list_top">
		...
		<!-- 카테고리 -->
		<jsp:include page="/category/list.ijw">
			<jsp:param name="qString" value="gregcode=${gvo.gregcode}&gsort=${gvo.gsort}"/>
		</jsp:include>
		<div class="reg_find">
			<form method="GET" action="/ijw/local.ijw" id="formQstr">
			...
			<div class="reg_area">
				...
				<a href="/ijw/regAreaMap.ijw" class="sitebtn">지도로 보기</a>
			</div>
			...
			<script type="text/javascript" src="/ijw/include/js/reglist.js"></script>
			<script>
...
<c:if test="${not empty param.gregcode}">
	...
	<c:if test="${param.gsort == '1' || param.gsort == '2'}">
		<div class="tab_sort"><a href="/ijw/local.ijw?gregcode=${param.gregcode}&cgnum=${param.cgnum}">전체</a></div>
	</c:if>
	<c:if test="${param.gsort == '1'}">
		<div class="tab_sort tab_saja tab_sel">구매</div>
	</c:if>
	<c:if test="${param.gsort != '1'}">
		<div class="tab_sort tab_saja"><a href="/ijw/local.ijw?gregcode=${param.gregcode}&gsort=1&cgnum=${param.cgnum}">구매</a></div>
	</c:if>
	<c:if test="${param.gsort == '2'}">
		<div class="tab_sort tab_panda tab_sel">판매</div>
	</c:if>
	<c:if test="${param.gsort != '2'}">
		<div class="tab_sort tab_panda"><a href="/ijw/local.ijw?gregcode=${param.gregcode}&gsort=2&cgnum=${param.cgnum}">판매</a></div>
	</c:if>
	</div>
</c:if>
	...
	<div class="list_grid">
		<ul>
			<c:forEach items="${generalList}" var="genvo">
				<li class="grid_item">
					<a href="/ijw/generalDetailForm.ijw?gnum=${genvo.gnum}">
						<div class="grid_item_box">
							...
							<c:if test="${genvo.gsort == '2'}">
								...
								<img src="/ijw/uploadImages/Thumb_${genvo.gphoto}">
							</div>
	...
	<c:if test="${param.gsort == '2'}">
		<div class="list_paging paging_panda">
			...
			<jsp:include page="paging.jsp" flush="true">
				<jsp:param name="url" value="/ijw/generalPList.ijw"/>
